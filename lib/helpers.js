...
function describeItNested(mochaInstance, command, message, callback, options) {
  if (command === 'it') return mochaInstance.it(message, callback);

  return mochaInstance.describe('', nestedFunc(mochaInstance, message, callback));
}
...
function nestedFunc(mochaInstance, message, callback) {
  return function () {
